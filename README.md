...
| `agentic-pm-eval` | Phase 3 | Pass rate with worst-slice variance and pass^k as the reliability gate, compound probability projection, semantic-vs-state validation, coverage statement, adversarial suite, LLM-as-judge calibration, model-version policy, all-owner readiness memo. |
