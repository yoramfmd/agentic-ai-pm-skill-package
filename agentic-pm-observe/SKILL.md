...
description: >
  Walk the PM through the Observe phase of the agentic PM lifecycle. Measures what the agent is actually doing in production. Produces: six observation instruments (task success, unintended action, override frequency, confidence calibration, rollback time, incident recovery), data-layer observation, operational guardrails (ceilings, kill switch, circuit breaker, burn-rate alert), deployment-event log, supervised-vs-bypass report, supervisory engagement metrics, stratified performance report. Trigger phrases: "monitor the agent", "observation instruments", "the agent is in production", "what should I be measuring", "kill switch", "burn-rate alert", "operational guardrails", "agent went off the rails", "agent is consuming too much", "override frequency", "data-layer observability", "supervised vs bypass". Also trigger when the team has shipped an agent, when a supervisor reports the agent feels off, or when a cost spike or behavioral anomaly appears.
---
