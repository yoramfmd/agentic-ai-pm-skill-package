...
description: >
  Strategic posture for agent behavior governance across a portfolio of agentic products. Cross-phase, cross-product. Not for a single agent. Names the discipline and the three-stage maturity arc (hand-built artifacts, consolidated control plane, platform primitive). Produces a portfolio maturity assessment and a consolidation roadmap. Trigger phrases: "agent behavior governance", "agentic behavior control plane", "across our agents", "portfolio of agents", "should we build a control plane", "consolidate agent guardrails", "platform vs team for agents", "we have multiple agents", "agent governance maturity", "how do we govern agents at scale". Use when a head of product is asking whether to keep building Channel 2 from scratch each time or to consolidate. Not for single-agent design (use agentic-pm-design) or single-agent eval (use agentic-pm-eval).
---
